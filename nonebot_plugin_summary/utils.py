...
from nonebot_plugin_session_orm import get_session_by_persist_id, SessionModel, get_session_persist_id
...
async def build_records(bot,event,records: List[MessageRecord])->str:
    ...
    for i in records:
        ...
        if not user_info:
            continue
        name = user_info.user_displayname if user_info.user_displayname else user_info.user_name if user_info.user_name else user_info.user_id
        msg = i.plain_text
        s += f"{name}:{msg}\n"
    return s

async def get_records(session:Session,number:int=plugin_config.default_context) -> List[MessageRecord]:
    where = [
